...
//		}
		...
		if (last.getEndPoint().epsilonEquals(inCurve.getEndPoint(), 
				GeometryConstants.EPSILON)
			|| last.getStartPoint().epsilonEquals(
				inCurve.getStartPoint(),
				GeometryConstants.EPSILON)) {
			addShape(inCurve.swap());
			return true;
		}
		if (last.getEndPoint().epsilonEquals(inCurve.getStartPoint(), 
				GeometryConstants.EPSILON)
			|| last.getStartPoint().epsilonEquals(
				inCurve.getEndPoint(),
				GeometryConstants.EPSILON)) {
...
//				Prof2d prof2d = (Prof2d) itr.next();
				prof2ds
					.setApplet(new edu.ohiou.labimp.basis.Draw2DApplet(prof2ds));
				prof2ds.display();
//			}
		} catch (Exception e) {
		}
