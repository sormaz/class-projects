...
public class Arc extends CurveSegment {
	...
	public Arc(double rad, Point3d cen,
		...
		// Create inverse matrix to transform Arc to 2D and 
		// to calculate start and end angles from start and end point.
		Matrix4d inverseMatrix = Ellipse.getInverseMatrix(cen, xAxisVec, yAxisVec, getNormal());
	...
	public Arc(Arc inArc) {
		this(inArc.getRadius(), inArc.getCenter(), inArc.getNormal(), 
				inArc.getStartAngle(), inArc.getEndAngle());
		if(inArc.hasAxes()) {
			this.xAxis = inArc.getXAxis();
			this.yAxis = inArc.getYAxis();
		}
	}
	 ...
	 *
	 */
	public void setNormal(Vector3d arcNormal) {
	...
	public Arc transform(Matrix4d transformMatrix) {
		...
		transformMatrix.transform(this.getXAxis(), newXAxis);
		transformMatrix.transform(this.getYAxis(), newYAxis);
		return new Arc(
				this.radius,
				newCenter,
				newXAxis,
	...
	public CurveSegment swap() {
		...
		nX.negate(this.getXAxis());
		Arc newArc = new Arc(radius, center, nX, this.getYAxis(), nStart, nEnd);
