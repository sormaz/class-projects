...
// CONSTRUCTORS
  ...
  public boolean isPointOnLineSegment (Point3d point) {
    try {
      ...
      double endFactor = line.factorForPoint(getEndPoint());
   ...
   public Line2d getLine2d(Vector3d projectionVector)
 {
   ...
   {
   	LineSegment line =  new LineSegment(this.getEndPoint(), this.getStartPoint());   	
   	return line;
   }
   ...
   */
   public Line2D.Double createDisplayLine(DrawWFPanel canvas) {
    return new Line2D.Double (canvas.createDisplayPoint(getStartPoint ()),
				canvas.createDisplayPoint(getEndPoint ()) );
   }
  ...
  public void paintComponent (Graphics2D g, DrawWFPanel canvas) {
    ViewObject.doNothing(this, "color is " + getColor());
   ...
   */
     public void repaint() {
    ((DrawWFPanel) this.getCanvas()).repaint();
  }
  ...
  public static void main (String args []) {
    ...
    //ls.setApplet();
    ls.setApplet(new DrawWFApplet (ls, new Point3d(1,1,1), 50));
