...
//  private AffineTransform initTransform;
  ...
  private HashMap<Color, HashSet<Shape>> drawMap = new HashMap<Color, HashSet<Shape>>();
  ...
  public Draw2DPanel(Drawable2D inTarget) throws IllegalArgumentException {
    this(inTarget, (Draw2DApplet) ((Viewable)inTarget).getApplet(), DEFAULT_TRANSFORM,
        DEFAULT_SCALE);
  }
...
//}
  ...
  public void setTarget(Drawable2D inTarget) {
    ...
    if (inTarget instanceof Viewable)
      applet = (Draw2DApplet) ((Viewable)inTarget).getApplet();
    computeBoundingBox();
...
//      System.out.println("shape" + shape);
    ...
    if (target != null) {
      ...
      for (Iterator itr = targets.iterator(); itr.hasNext();) {
        ...
        if (o instanceof Drawable2D) {
          ...
          this.targetTable.put(selectable.getPosition(), selectable);
        }
  ...
  public void makeDrawSets() {
    drawMap = new HashMap<Color, HashSet<Shape>>();
...
//  viewXformLabel = new JLabel("" + printText);
    ...
    toolBar.add(panLeftButton);
    toolBar.add(panRightButton);
    toolBar.add(panUpButton);
    toolBar.add(panDownButton);
    toolBar.add(panAllButton);
    toolBar.add(rotateCWButton);
    toolBar.add(rotateCCWButton);
    toolBar.add(saveButton);
    toolBar.add(printButton);
    //    JPanel buttonSetPanel = new JPanel ();
    //    buttonSetPanel.setLayout(new BorderLayout());
    //    buttonSetPanel.add (buttonPanel, BorderLayout.WEST);
    ...
    protected void paintComponent(Graphics2D g) {
      ...
      if (paintImages) {
        ...
        for (Iterator itr = images.iterator(); itr.hasNext();) {
          ...
          boolean done = g.drawImage(image, xform, null);
          System.out.println("image doen is: " + done);
        }
      }
...
//          Rectangle rect = shape.getBounds();
      ...
      for (Iterator colorItr = colors.iterator(); colorItr.hasNext();) {
        Color currentColor = (Color) colorItr.next();
        HashSet shapeSet = (HashSet) drawMap.get(currentColor);
        for (Iterator shapeItr = shapeSet.iterator(); shapeItr.hasNext();) {
          Shape currentShape = (Shape) shapeItr.next();
...
//      ex.printStackTrace();
    ...
    public void mouseDragged(MouseEvent e) {
      if (mouseMode == Drawable2D.MODIFY_VIEW) {
        ...
        System.out.println ("selected>" + selectedObject.toString());
        ...
        try {
          ...
          if (selectedObject instanceof Drawable2D) {
            ((Drawable2D) selectedObject).setPosition(mousePoint);
          }
...
//  try { printJob.print(); } catch (Exception PrintException) { }
  ...
  public void scaleFitTarget() {
    if (boundingBox != null) {
      ...
      System.out.println(" old x scale " + oldXScale);
      System.out.println(" old Y scale " + oldYScale);
